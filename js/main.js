...
(function () {
  ...
  function initForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      const message = form.querySelector('[data-field="message"] textarea').value.trim();
      const service = form.getAttribute("data-service") || "General Enquiry";
      const fullPhone = code + " " + phone;

      // Params — matched to your "Contact Us" EmailJS template variables
      const params = {
        name: fullName,
        from_name: fullName,
        from_email: email,
      ...
      if (tourField) {
        ...
        params.participants = adults + " adult(s), " + children + " child(ren)";
        params.message +=
          "\nTour plan: " + tour +
          "\nPreferred date: " + tourDate +
          "\nAdults: " + adults +
          "\nChildren: " + children +
          (ages.length ? "\nChildren's ages: " + ages.join(", ") : "");
      }
      ...
      if (vehicleField) {
        ...
        const rDaysEl = form.querySelector('[name="rental_days"]');
        const rDateEl = form.querySelector('[name="rental_date"]');
        const rDays = rDaysEl ? rDaysEl.value : "";
        const rDate = rDateEl ? rDateEl.value : "";
